...
local function Shared(self, unit)
	...
	self:HighlightUnit("Mouseover", 1, 0, 0)
	...
	health:Height(28*C["raidframes"].gridscale*T.raidscale)
	...
	health.value:SetFont(font2, 11*C["raidframes"].gridscale*T.raidscale, "THINOUTLINE")
	...
	power:SetHeight(3*C["raidframes"].gridscale*T.raidscale)
	...
	name:SetPoint("LEFT") 
	name:SetPoint("RIGHT")
	name:SetFont(font2, 12*C["raidframes"].gridscale*T.raidscale)
	self:Tag(name, "[Tukui:getnamecolor][Tukui:nameshort]")
	self.Name = name
	
    if C["raidframes"].aggro == true then
	...
	if C["raidframes"].showsymbols == true then
	...
	ReadyCheck:Height(12*C["raidframes"].gridscale*T.raidscale)
	ReadyCheck:Width(12*C["raidframes"].gridscale*T.raidscale)
	...
	if not C["raidframes"].raidunitdebuffwatch == true then
	...
	if C["raidframes"].healcomm then
		...
		if C["raidframes"].gridhealthvertical then
			...
			mhpb:Width(66*C["raidframes"].gridscale*T.raidscale)
			mhpb:Height(50*C["raidframes"].gridscale*T.raidscale)
		else
		...
		if C["raidframes"].gridhealthvertical then
			...
			ohpb:Width(66*C["raidframes"].gridscale*T.raidscale)
			ohpb:Height(50*C["raidframes"].gridscale*T.raidscale)
		else
			ohpb:SetPoint('TOPLEFT', mhpb:GetStatusBarTexture(), 'TOPRIGHT', 0, 0)
			ohpb:SetPoint('BOTTOMLEFT', mhpb:GetStatusBarTexture(), 'BOTTOMRIGHT', 0, 0)
			ohpb:Width(6*C["raidframes"].gridscale*T.raidscale)
		end
	...
	if C["raidframes"].raidunitdebuffwatch == true then
		...
		RaidDebuffs:Height(24*C["raidframes"].gridscale)
		RaidDebuffs:Width(24*C["raidframes"].gridscale)
		...
		if C["auras"].auratimer then
			...
			if not C["raidframes"].cooldown then
				RaidDebuffs.cd.noOCC = true -- remove this line if you want cooldown number on it
			end
		end
		
		RaidDebuffs.count = RaidDebuffs:CreateFontString(nil, 'OVERLAY')
		RaidDebuffs.count:SetFont(C["media"].uffont, 9*C["raidframes"].gridscale, "THINOUTLINE")
		RaidDebuffs.count:SetPoint('BOTTOMRIGHT', RaidDebuffs, 'BOTTOMRIGHT', 0, 2)
		RaidDebuffs.count:SetTextColor(1, .9, 0)
		
		RaidDebuffs:FontString('time', C["media"].uffont, 9*C["raidframes"].gridscale, "THINOUTLINE")
...
oUF:Factory(function(self)
	...
	if C["raidframes"].gridonly ~= true then
		local raid = self:SpawnHeader("TukuiGrid", nil, "custom [@raid16,exists] show;hide",
			...
			'initial-width', T.Scale(66*C["raidframes"].gridscale*T.raidscale),
			'initial-height', T.Scale(50*C["raidframes"].gridscale*T.raidscale),
			"showRaid", true,
			"xoffset", T.Scale(3),
			"yOffset", T.Scale(-3),
			"point", "LEFT",
		...
		local pets = {} 
			...
			pets[1]:Point('TOPLEFT', raid, 'TOPLEFT', 0, -50*C["raidframes"].gridscale*T.raidscale + -3)
			pets[1]:Size(66*C["raidframes"].gridscale*T.raidscale, 50*C["raidframes"].gridscale*T.raidscale)
		for i =2, 4 do 
			pets[i] = oUF:Spawn('partypet'..i, 'oUF_TukuiPartyPet'..i) 
			pets[i]:Point('LEFT', pets[i-1], 'RIGHT', 3, 0)
			pets[i]:Size(66*C["raidframes"].gridscale*T.raidscale, 50*C["raidframes"].gridscale*T.raidscale)
		end

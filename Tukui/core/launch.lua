...
local function chatsetup()
	...
	for i = 1, NUM_CHAT_WINDOWS do
		...
		frame:Size(C["panels"].infowidth + 1, C["chat"].height)
		
		-- tell wow that we are using new size
		SetChatWindowSavedDimensions(chatFrameId, T.Scale(C["panels"].infowidth + 1), T.Scale(C["chat"].height))
		
		-- move general bottom left or Loot (if found) on right
		if i == 1 then
			frame:ClearAllPoints()
			frame:Point("BOTTOMLEFT", TukuiInfoLeft, "TOPLEFT", 0, 6)
		elseif i == 4 and C["chat"].rightchat then
			frame:ClearAllPoints()
		...
		if i == 1 then FCF_SetWindowName(frame, "일반") end
		if i == 2 then FCF_SetWindowName(frame, "전투") end
	end

...
T.PostUpdateHealth = function(health, unit, min, max)
	if not UnitIsConnected(unit) or UnitIsDead(unit) or UnitIsGhost(unit) then
		...
		if min ~= max then
			...
			if C["unitframes"].showperhp == true and (unit == "player" or unit == "target") then
	...
	elseif UnitIsDead(unit) or UnitIsGhost(unit) then
		...
		if min ~= max then
			if pType == 0 then
				...
				elseif (unit and unit:find("boss%d")) and C["raidframes"].showperpw then
					power.value:SetFormattedText("%d%%", floor(min / max * 100))
				elseif (unit and unit:find("arena%d")) or unit == "focus" or unit == "focustarget" or (unit and unit:find("boss%d")) then
...
T.PostCreateAura = function(element, button)
	...
	button.remaining = T.SetFontString(button, C["media"].font, C["auras"].auratextscale, "THINOUTLINE")
...
local CheckInterrupt = function(self, unit)
	...
	if self.interrupt and UnitCanAttack("player", unit) then
		...
		self:SetStatusBarColor(unpack(C["castbar"].defaultcolor))
	end
end
...
T.EclipseDisplay = function(self, login)
	...
	if eb:IsShown() then
		...
		if self.Buffs then self.Buffs:ClearAllPoints() self.Buffs:SetPoint("BOTTOM", self, "TOP", 1, 12) end
	else
		txt:Hide()
		self.FlashInfo:Show()
		self.shadow:Point("TOPLEFT", -4, 4)
		if self.Buffs then self.Buffs:ClearAllPoints() self.Buffs:SetPoint("BOTTOM", self, "TOP", 1, 4) end
	end
end

T.ComboDisplay = function(self, event, unit)
...
T.MushroomDisplay = function(self)
	for i = 1, 3 do
...
T.countOffsets = {
	TOPLEFT = {6*C["raidframes"].gridscale, 1},
	TOPRIGHT = {-6*C["raidframes"].gridscale, 1},
	BOTTOMLEFT = {6*C["raidframes"].gridscale, 1},
	BOTTOMRIGHT = {-6*C["raidframes"].gridscale, 1},
	LEFT = {6*C["raidframes"].gridscale, 1},
	RIGHT = {-6*C["raidframes"].gridscale, 1},
	TOP = {0, 0},
	BOTTOM = {0, 0},
}
...
T.createAuraWatch = function(self, unit)
	...
	if (buffs) then
		for key, spell in pairs(buffs) do
			...
			icon:Width(6*C["raidframes"].gridscale)
			icon:Height(6*C["raidframes"].gridscale)
...
if C["raidframes"].raidunitdebuffwatch == true then
	...
	do
		T.buffids = {
			...
			DRUID = {
				...
				{740, "RIGHT", {0.6, 0.8, 1}, true}, -- Tranquility
			},
			PALADIN = {
				{53563, "TOPLEFT", {0.7, 0.3, 0.7}}, -- Beacon of Light
				{70940, "BOTTOMRIGHT", {1, 1, 0.6}, true}, -- Divine Guardian
			},
			SHAMAN = {
			...
			DEATHKNIGHT = {
				{51052, "BOTTOMRIGHT", {0.1, 0, 0.3}, true}, -- Anti-Magic Zone
			},
			ALL = {
				-- Death Knight
				{55233, "LEFT", {0.7, 0, 0}, true}, -- Vampiric Blood
				{48792, "LEFT", {0.3, 0.4, 0.7}, true}, -- Icebound Fortitude
				{48707, "LEFT", {0.6, 0.9, 0.2}, true}, -- Anti-Magic Shell
				{49039, "LEFT", {0.8, 0.8, 0.6}, true}, -- Lichborne
				-- Druid
				{22812, "LEFT", {0.5, 0.3, 0.1}, true}, -- Barkskin
				{22842, "LEFT", {0.7, 0.1, 0}, true}, -- Frenzied Regeneration
				{61336, "LEFT", {0.7, 0.7, 0.4}, true}, -- Survival Instincts
				{29166, "RIGHT", {0, 0.1, 0.6}, true}, -- Innervate
				-- Hunter
				{19263, "LEFT", {0.4, 0.8, 1}, true}, -- Deterrence
				-- Mage
				{45438, "LEFT", {0, 0.4, 0.8}, true}, -- Ice Block
				-- Paladin
				{31850, "LEFT", {1, 0.8, 0.2}, true}, -- Ardent Defender
				{498, "LEFT", {1, 0.6, 0.6}, true}, -- Divine Protection
				{86659, "LEFT", {1, 0.6, 0.4}, true}, -- Guardian of Ancient Kings					
				{1022, "RIGHT", {0.4, 0.4, 0.8}, true}, -- Hand of Protection
				{1044, "RIGHT", {0.8, 0.2, 0}, true}, -- Hand of Freedom
				{6940, "RIGHT", {0.6, 0, 0}, true}, -- Hand of Sacrafice
				{1038, "RIGHT", {1, 0.8, 0.6}, true}, -- Hand of Salvation
				-- Priest
				{47585, "LEFT", {0.6, 0.2, 0.8}, true}, -- Dispersion
				{10060, "RIGHT", {1, 0.8, 0.2}}, -- Power Infusion
				{33206, "RIGHT", {0.9, 0.8, 0.7}, true}, -- Pain Suppression
				{47788, "RIGHT", {0.7, 0.8, 0.9}, true}, -- Guardian Spirit
				-- Rogue
				{31224, "LEFT", {0.4, 0.2, 0.6}, true}, -- Cloak of Shadows
				{1966, "LEFT", {0.2, 0.6, 1}, true}, -- Feint
				{5277, "LEFT", {1, 0.8, 0.8}, true}, -- Evasion
				-- Shaman
				{30823, "LEFT", {1, 0, 0.2}, true}, -- Shamanistic Rage
				-- Warlock
				-- Warrior
				{871, "LEFT", {0.8, 0.6, 0.4}, true}, -- Shield Wall
				{12975, "LEFT", {1, 0.4, 0}, true}, -- Last Stand
				{55694, "LEFT", {0.8, 0.2, 0}, true}, -- Enraged Regeneration
				-- etc
	...
	do
		...
		-- Hunter
			...
			--SpellName(60210), --Freezing Arrow Effect
		...
		-- Mage
			...
			SpellName(18469), --Silenced - Improved Counterspell
		...
		-- Rogue
			...
			SpellName(6770), --Sap
			SpellName(18425), --Silenced - Improved Kick
		-- Shaman
			SpellName(51514), --Hex
		...
		-- Racial
			SpellName(25046), --Arcane Torrent
			SpellName(20549), --War Stomp
		--PVE
		}
		...
		ORD:RegisterDebuffs(T.arenaids)
	end
end

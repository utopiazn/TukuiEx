...
function Stuffing:Layout(lb)
	...
	if lb then
		bs = bags_BANK
		if C["panels"].infowidth >= 405 then
			cols = 11
		elseif C["panels"].infowidth >= 370 and C["panels"].infowidth < 405 then
			cols = 10
		elseif C["panels"].infowidth >= 335 and C["panels"].infowidth < 370 then
			cols = 9
		else
			cols = 8
		end
...
local function StuffingSlashCmd(Cmd)
	...
	if (cmd == "sort") or (cmd == "분류") then
		Stuffing_Sort(args)
	elseif (cmd == "psort") or (cmd == "특수물품분류") or (cmd == "특수분류") then
		Stuffing_Sort("c/p")
	elseif (cmd == "stack") or (cmd == "정리") then
		Stuffing:SetBagsForSorting(args)
		Stuffing:Restack()
	elseif (cmd == "test") or (cmd == "테스트") or (cmd == "시험") then
		Stuffing:SetBagsForSorting(args)
	elseif (cmd == "purchase") or (cmd == "구매") or (cmd == "구입") then
		-- XXX
		if Stuffing.bankFrame and Stuffing.bankFrame:IsShown() then
			...
			if (args == "yes") or (args == "예") or (args == "네") then
				PurchaseSlot()
				return
			end
		...
		Print("sort, 분류 - " .. L.bags_sort)
		Print("stack, 정리 - " .. L.bags_stack)
		Print("purchase yes, 구매 예 - " .. L.bags_buybankslot)
	end
end

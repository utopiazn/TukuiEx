...
local function RaidMarkCanMark()
	if GetNumRaidMembers() > 0 then
		if IsRaidLeader()or IsRaidOfficer()then
			...
			UIErrorsFrame:AddMessage("전술 목표 아이콘을 설정할 권한이 없습니다.", 1.0, 0.1, 0.1, 1.0, UIERRORS_HOLD_TIME)
			return false
		end	
	else
		return true
...
for i = 1, 8 do
	...
	Button.Texture:SetTexture("Interface\\AddOns\\Tukui\\medias\\textures\\raidicons.blp")

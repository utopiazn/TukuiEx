...
function openMail(index)
	if not InboxFrame:IsVisible() then return stopOpening(L.core_tukui..L.mail_needmailbox) end
	if index == 0 then MiniMapMailFrame:Hide() stopOpening(L.core_tukui..L.mail_complete) return end
	...
	if (numItems and numItems > 1) or (items > 1 and index <= items) then
		...
		stopOpening(L.core_tukui..L.mail_complete)
...
function onEvent(frame, event, arg1, arg2, arg3, arg4)
	if event == "UI_ERROR_MESSAGE" then
		if arg1 == ERR_INV_FULL then
			stopOpening(L.core_tukui..L.mail_invfull)
		end
	end
end
...
button:SetScript("OnEnter", function()
	...
	GameTooltip:AddLine(string.format("%d"..L.mail_message, GetInboxNumItems()), 1, 1, 1)
	GameTooltip:Show()
end)
...
function copper_to_pretty_money(c)
	if c > 10000 then
		return ("%d"..L.goldabbrev.." %d"..L.silverabbrev.." %d"..L.copperabbrev):format(c/10000, (c/100)%100, c%100)
	elseif c > 100 then
		return ("%d"..L.silverabbrev.." %d"..L.copperabbrev):format((c/100)%100, c%100)
	else
		return ("%d"..L.copperabbrev):format(c%100)
	end
end

	...
	if not bind.loaded then
		...
		function bind:Update(b, spellmacro)
			...
			if spellmacro=="SPELL" then
				...
				GameTooltip:SetScript("OnHide", function(self)
					...
					if #bind.button.bindings == 0 then
						self:AddLine(L.bind_nobind, .6, .6, .6)
					else
						self:AddDoubleLine(L.bind_binding, L.bind_key, .6, .6, .6, .6, .6, .6)
			...
			elseif spellmacro=="MACRO" then
				...
				bind.button.bindings = {GetBindingKey(spellmacro.." "..bind.button.name)}
					if #bind.button.bindings == 0 then
						GameTooltip:AddLine(L.bind_nobind, .6, .6, .6)
					else
						GameTooltip:AddDoubleLine(L.bind_binding, L.bind_key, .6, .6, .6, .6, .6, .6)
						for i = 1, #bind.button.bindings do
							GameTooltip:AddDoubleLine(L.bind_binding..i, bind.button.bindings[i], 1, 1, 1)
						end
					end
				GameTooltip:Show()
...
SLASH_MOUSEOVERBIND3 = "/단축키"
SLASH_MOUSEOVERBIND4 = "/단축키설정"
SLASH_MOUSEOVERBIND5 = "/ㅠㅑㅜ아됴"
SLASH_MOUSEOVERBIND6 = "/ㅙㅍㄷ규ㅑㅜㅇ"
if not IsAddOnLoaded("HealBot") then
	SLASH_MOUSEOVERBIND7 = "/hb"
	SLASH_MOUSEOVERBIND8 = "/ㅗㅠ"
end
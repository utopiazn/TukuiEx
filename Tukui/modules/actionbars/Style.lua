...
local function style(self)
	...
	Btname:Point("BOTTOMLEFT", 0, 2)
	Btname:SetFont(C["media"].font, 12, "OUTLINE")
	Btname.ClearAllPoints = T.dummy
	Btname.SetPoint = T.dummy
 
	if not C["actionbar"].btname == true then
		Btname:SetText("")
		Btname:Kill()
	end	
...
local function updatehotkey(self, actionButtonType)
	...
	-- for Korean WoW client
	text = replace(text, '(숫자패드 )', 'N')
	text = replace(text, '(번 마우스 버튼)', 'M')
	text = replace(text, '(마우스 가운데 버튼)', '3M')
	text = replace(text, '(마우스 휠 위로)', 'MU')
	text = replace(text, '(마우스 휠 아래로)', 'MD')

...
local difficultyInfo = { L.datatext_normal, L.datatext_normal, L.datatext_hard, L.datatext_hard }
...
local function formatResetTime(sec,table)
	...
	local string = gsub(gsub(format(" %d"..L.time_day.." %d"..L.time_hour.." %d"..L.time_minute.." "..((d==0 and h==0) and "%d"..L.time_second or ""),d,h,m,s)," 0[dhms]"," "),"%s+"," ")
	local string = strtrim(gsub(string, "([dhms])", {d=table.days or L.time_day,h=table.hours or L.time_hour,m=table.minutes or L.time_minute,s=table.seconds or L.time_second})," ")
	return strmatch(string,"^%s*$") and "0"..(table.seconds or L..L.time_second) or string
end

...
local function LoadSkin()
	...
	local function SkinBags()	
		for i=1, NUM_CONTAINER_FRAMES, 1 do
			...
			if container and not container.backdrop then
				...
				container:CreateBackdrop("Default")
	...
	hooksecurefunc("BankFrameItemButton_Update", function(button)
		if not BankFrame.backdrop then
			...
			BankFrame:CreateBackdrop("Default")
		...
		if highlight and not highlight.skinned then
			highlight:SetTexture(unpack(C["media"].bordercolor), 0.3)
		...
		for index, frameName in ipairs(ContainerFrame1.bags) do
			...
			if ( index == 1 ) then
				...
				if C["chat"].background then
					frame:SetPoint("BOTTOMRIGHT", TukuiChatBackgroundRight, "TOPRIGHT", 2, 3 );
				else
					frame:SetPoint("BOTTOMRIGHT", TukuiInfoRight, "TOPRIGHT", 2, 3 );
				end

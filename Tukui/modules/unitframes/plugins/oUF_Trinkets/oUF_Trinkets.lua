...
if C["unitframes"].enable ~= true or C["unitframes"].arenaframes ~= true then return end
	...
	if self.endTime < GetTime() then
		...
		if unit and arenaFrame[unit] then
			if arenaFrame[unit].Trinket.trinketUpAnnounce then
				SendChatMessage(L.unitframes_ouf_trinketready..UnitName(unit).." "..UnitClass(unit), "PARTY")
			end
		end
...
local TrinketUsed = function(guid, time)
	...
	if unit and arenaFrame[unit] then
		...
		if arenaFrame[unit].Trinket.trinketUseAnnounce then
			message = time == 120 and L.unitframes_ouf_trinketused or L.unitframes_ouf_wotfused
